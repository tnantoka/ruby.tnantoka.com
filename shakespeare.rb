...
class Population
  ...
  def initialize(target, mutation_rate, num)
    @target = target
  ...
  def all_phrases
    display_limit = [@population.size, 50].min
...
def display_info
  ...
  puts "Best phrase: #{answer}"
  puts "total generations: #{@population.generations}"
  puts "average fitness:   #{@population.average_fitness}"
  puts "total population:  #{@popmax}"
  puts "mutation rate:     #{(@mutation_rate * 100).to_i}"
  puts "All phrases:\n#{@population.all_phrases.split('\n')}"
  puts
end

def answer
  @population.compute_best
end
